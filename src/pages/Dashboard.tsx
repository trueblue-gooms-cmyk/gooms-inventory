...
export default function Dashboard() {
  ...
  const calculateRealMetrics = () => {
    ...
    inventory.forEach((item: any) => {
      const product = products.find((p: any) => p.id === item.product_id);
      if (product) {
        const productType = (product as any).product_type || (product as any).type;
        if (productType && typeStats[productType as keyof typeof typeStats]) {
          const typeKey = productType as keyof typeof typeStats;
